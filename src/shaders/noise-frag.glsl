...
void main() {
  ...
  // vec3 color = vec3(1.0, 1.0, 1.0) * cos( vPosition.x + cnoise(10.0* vPosition + vec3(vUv, cos(uTime) + 1.0)  ));
  
  float g = cnoise(vPosition * ((cos(0.1 * uTime) + 1.0) * 0.5 + 0.5) ) * 20.0;
  float grain = g - floor(g);

  vec3 color = vec3(0.667, 0.424, 0.224) * grain;
