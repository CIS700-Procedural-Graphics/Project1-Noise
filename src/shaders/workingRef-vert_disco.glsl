...
float interpNoiseByCorners(float x, float y, float z) {
    ...
    float val = -0.5 + abs(cosInterpFunc(b, f, zDiff));
...
void main() {
  ...
  float pixelImageHeight = 664.0;
    float noise = .5 * interpNoiseByCorners(position[0] * time * 5.0, position[1] * time * 5.0, position[2] * time * 5.0);
    float height = 50.0 * cosInterpFunc(1.0, 664.0 - pixelImageHeight, noise);
    gl_Position = projectionMatrix
    				* modelViewMatrix
    				* ( vec4(position, 1.0) + 
    					 123456789.0 * height * normalize (vec4(nor[0], nor[1], nor[2], 1.0))
    				);
   ...
   //         );
     ...
     */
     ...
     float height_color = cosInterpFunc(1.0, 664.0 - pixelImageHeight, 3.0 * noise_color);
     vUv = vec2 (20.0, height_color);
