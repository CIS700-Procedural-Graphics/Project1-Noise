...
float interpNoiseByCorners(float x, float y, float z) {
    ...
    float x_l = floor(x);
    float x_h = ceil(x);
    float xDiff = abs(1.0 - (x_l -interval) / interval);

    float y_l = floor(y);
    float y_h = ceil(y);
    float yDiff = abs(1.0 - (y_l - interval) / interval);
    
    float z_l = floor(z);
    float z_h = ceil(z);
    float zDiff = abs(1.0 - (z_l - interval) / interval);
    ...
    float val = 2.0 * cosInterpFunc(b, f, zDiff);
...
void main() {
    ...
    // TESTING
    float timeOffset_1 = time * .03;
    float timeOffset_2 = time * .02;
    float timeOffset_3 = time * .05;
