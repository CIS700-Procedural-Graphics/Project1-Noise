...
float noiseFunc_4(float in1, float in2, float in3) {
    vec3 vec = vec3(in1, in2, in3);
    return  0.0000005 * (dot ((vec - 1.79284291400159 - 0.85373472095314 * floor(vec * (1.0 / 289.0)) * 289.0), vec));
}
...
float currNoiseFunc(float in1, float in2, float in3) {
    return noiseFunc_4(in1, in2, in3);
}
...
float overallNoiseFunc(float in1, float in2, float in3) {
    ...
    float noise = smooth(in1, in2, in3);
...
float interpNoiseByCorners(float x, float y, float z) {
    ...
    float x_l = floor(x);
    float x_h = ceil(x);
    float xDiff = 1.0 - abs(interval - x_l) / interval;

    float y_l = floor(y);
    float y_h = ceil(y);
    float yDiff = 1.0 - abs(interval - y_l) / interval;
    
    float z_l = floor(z);
    float z_h = ceil(z);
    float zDiff = 1.0 - abs(interval - z_l) / interval;
    ...
    // interp between them
    float bf = cosInterpFunc(ltf, rtf, xDiff);
    float bb = cosInterpFunc(ltb, rtb, xDiff);
...
void main() {
    ...
    // TESTING
    float timeOffset_1 = time * -0.3;
    float timeOffset_2 = time * .13;
    float timeOffset_3 = time * .03;

    float noise = interpNoiseByCorners(position[0] * timeOffset_1, position[1] * timeOffset_2, position[2] * timeOffset_3);
   ...
   //         );
     ...
     */
     ...
     // FOR TESTING
     float pixelImageHeight = 664.0;
     float noise_color = smooth(position[0] + time, position[1] + time, position[2] + time);
     float height = cosInterpFunc(1.0, 664.0 - pixelImageHeight, 3.0 * noise_color);
