...
//	in3 = in3 * 1.9;
    ...
    // return (sin( dot( vec3(in1, in2, in3), vec3(80.233, .9898, 50.3535))) ); a;sldkjfa;lskdjfa;sldkfj;alksdjf
    return (sin( dot( vec3(in1, in2, in3), vec3(12.9898, 78.233, 45.3535))) );
...
float overallNoiseFunc(float in1, float in2, float in3) {
	// EDITS HERE BETWEEN ORIG FIRE AND MOVING BLOOPS a;slkdfja;sldkjfa;sldkfja;sldkfja;sldkfja;sldkfj;alskdfj
//	in1 = in1*.005;
//	in2 = in2*.04;
//	in3 = in3*.006;
...
void main() {
    ...
    // TO MAKE MOVE WITH UNDULATION
     ...
     float noise = overallNoiseFunc(position[0] + timeOffset, position[1] + time, position[2] + timeOffset);
     noise = noise / 50.0;
