...
float interpNoiseByCorners(float x, float y, float z) {
    ...
    float xHigh = xLow + xDist;
    float yHigh = yLow + yDist;
    float zHigh = zLow + zDist;
...
float overallNoiseFunc(float in1, float in2, float in3) {
    ...
    for (int j = 0; j < numOctaves; j++) {
        ...
        total += interpNoiseByCorners(in1, in2, in3) * frequency;
    }
...
void main() {
    ...
    // TESTING
    float noise = overallNoiseFunc(position[0], position[1], time);
    
    float noise_1 = 0.003*noise;
    float noise_2 = 0.007*noise;
    float noise_3 = 0.005*noise;
     ...
     */
     ...
     // FOR TESTING
     float pixelImageHeight = 664.0;
     float height = cosInterpFunc(1.0, 664.0 - pixelImageHeight, noise);
