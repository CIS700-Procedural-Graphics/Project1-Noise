...
float noise_3(float x, float y, float z) {
	float n = x * 109277.101 + x * 107.0 ; 
	float m = y * 101010010.0001; 
	n = fract(cos(dot(vec2(n,z), vec2(1002946.294485, 128282.9383))) * 1094877.1293);
	n = fract(cos(dot(n, m)));
	return n;
}
//----------------------------
...
float interpolateNoise(float x, float y, float z, int i) {
	...
	if (i == 0) {
		 ...
		 v0 = noise_3(p0.x, p0.y, p0.z);
		 v1 = noise_3(p1.x, p1.y, p1.z);
		 v2 = noise_3(p2.x, p2.y, p2.z);
		 v3 = noise_3(p3.x, p3.y, p3.z);
		 v4 = noise_3(p4.x, p4.y, p4.z);
		 v5 = noise_3(p5.x, p5.y, p5.z);
		 v6 = noise_3(p6.x, p6.y, p6.z);
		 v7 = noise_3(p7.x, p7.y, p7.z);
	}
